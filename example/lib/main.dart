...
class _HomePageState extends State<HomePage> {
  ...
  void _onCameraFrame(CvImage frame) {
    try {
      final processed = ImageProcessingService.applyFilter(frame, _activeFilter);
      if (processed != frame) {
        frame.dispose();
      }
  ...
  Future<void> _applyCurrentFilter() async {
    try {
      ...
      final workingCopy = ImageProcessingService.cloneImage(_originalImage!);
      final result = ImageProcessingService.applyFilter(workingCopy, _activeFilter);
      ...
      final bytes = ImageProcessingService.encodeImage(_processedImage!);
      setState(() {
        _displayBytes = bytes;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Flutter OpenCV Demo'),
        actions: [
          Center(
            child: Padding(
