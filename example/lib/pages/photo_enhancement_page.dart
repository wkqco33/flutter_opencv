...
class PhotoEnhancementPage extends StatefulWidget {
  const PhotoEnhancementPage({super.key});

  @override
  State<PhotoEnhancementPage> createState() => _PhotoEnhancementPageState();
}

class _PhotoEnhancementPageState extends State<PhotoEnhancementPage> {
  static const String _tag = 'PhotoEnhancementPage';
  ...
  void _onCameraFrame(CvImage frame) {
    try {
      final processed = ImageProcessingService.processPhotoEnhancement(frame);
  ...
  Future<void> _processImage() async {
    ...
    // Isolate에서 사진 품질 개선 파이프라인 처리
    final resultBytes =
        await ImageProcessingService.processPhotoEnhancementIsolated(
          originalBytes,
        );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('사진 품질 개선')),
      body: Column(
        children: [
          Expanded(
            child: ImageViewer(
