...
class _DocumentScannerPageState extends State<DocumentScannerPage> {
  static const String _tag = 'DocumentScannerPage';
  ...
  void _onCameraFrame(CvImage frame) {
    try {
      final processed = ImageProcessingService.processDocumentScanner(frame);
  ...
  Future<void> _processImage() async {
    ...
    // Isolate에서 문서 스캐너 파이프라인 처리
    final resultBytes =
        await ImageProcessingService.processDocumentScannerIsolated(
          originalBytes,
        );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('문서 스캐너')),
      body: Column(
        children: [
          Expanded(
            child: ImageViewer(
          ...
          Container(
            ...
            child: const Text(
              '처리 과정: Grayscale → Gaussian Blur → Adaptive Threshold → Morph Open',
